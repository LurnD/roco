...
BuildGui() {
    ...
    guiObj := Gui("+AlwaysOnTop", "洛克王国循环助手 (Ctrl+= 启停)")
...
RunInnerCycle(hwnd, index, total) {
    ...
    for _, key in parts {
        ...
        SetStatus("状态：内循环 " index "/" total " - 发送 " keyName)
        Loop presses {
            if !app.isRunning
                return false
...
ValidateSettings(settings) {
    ...
    loopMin := ParsePositiveInteger(settings.loopMin, "循环次数最小值")
    loopMax := ParsePositiveInteger(settings.loopMax, "循环次数最大值")
    if loopMin > loopMax
